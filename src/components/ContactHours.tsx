...
export default async function ContactHours() {
  const t = await getTranslations("Contact");

  const whatsappHref = `https://wa.me/${clinicInfo.whatsapp}?text=${encodeURIComponent(
    t("whatsappMessage", { clinicName: clinicInfo.name }),
  )}`;

  // Mirrors clinic-data.ts's `hours` array (Sunday–Thursday open, Friday
  // and Saturday closed) but built from translated strings instead of
  // that English-only static data — see closedWeekdays in clinic-data.ts
  // for the source of truth this display has to stay in sync with.
  const hours = [
    { day: t("hoursWeekdays"), time: t("hoursTime") },
      ...
      <div className="grid gap-10 sm:grid-cols-2 sm:gap-6">
        <div className="rounded-xl border border-border bg-surface p-8 shadow-sm">
          ...
          <dl className="mt-5 space-y-3 text-sm">
            ...
            <div className="flex justify-between gap-4 text-end">
